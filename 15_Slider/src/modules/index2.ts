...
function idGen(elem: HTMLElement, valu: string): string {
...
// setting and returns new values for new "list item"
function setNewElem(val: string) {
    ...
    return [newElem, delBtn, rewriteBtn, newId, itemIndex];
};

function removeElem(newElem: HTMLElement) {
    newElem.remove();
...
function delElemInStorage(itemIndex: number) {
...
function addDelBtnFunc(delBtn: HTMLButtonElement, newElem: HTMLElement, itemIndex: number) {
    // adding event to del-btn witch delete its own elem
    delBtn.addEventListener('click', (ev) => {
        ev.preventDefault();

        // =================================== removing item from html container
        removeElem(newElem);
        ...
        delElemInStorage(itemIndex);
    });
};

function rewriteValItem(title: HTMLElement) {
    // getting text input
...
function rewriteLocalStorage(itemIndex: number, newId: string, valueNew: string) {
...
function eventRewriteItem(this: Function ,ev: Event, title: HTMLElement, itemIndex: number, newId: string) {
    // stopping refresh site
    ev.preventDefault();
    ev.stopImmediatePropagation();

    const valueNew = rewriteValItem(title);

    // ============================================== alert animation
    alertRewriteAnim();

    // ================================================ Getting old arr from local storage
    rewriteLocalStorage(itemIndex, newId, valueNew);

    // ================================================================
    // removing this func from event in btn
    submit__btn.removeEventListener('click', eventRewriteItem.bind(null, ev, title, itemIndex, newId));
...
function listItem(val: string) {
    ...
    const newElemArr = setNewElem(val);
    const
        newElem: HTMLElement = newElemArr[0] as HTMLElement,
        delBtn: HTMLButtonElement = newElemArr[1] as HTMLButtonElement,
        rewriteBtn: HTMLButtonElement = newElemArr[2] as HTMLButtonElement,
        newId: string = newElemArr[3] as string,
        itemIndex: number = newElemArr[4] as number;

    // ========================================== delete button func 
        
    // getting title
    const title: HTMLElement = newElem.querySelector('.grocery-item__title') as HTMLElement;

    addDelBtnFunc(delBtn, newElem, itemIndex);
    ...
    rewriteBtn.addEventListener('click', (ev) => {
        ...
        submit__btn.addEventListener('click', eventRewriteItem.bind(null, ev, title, itemIndex, newId));
    });
};
